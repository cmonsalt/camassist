...
4. PERSUASIÓN: Crea deseo pintando fantasías sensuales. NUNCA menciones precio/${currencyTerm} a menos que el fan pregunte "how much/cuánto cuesta" directo. "Qué haces en pvt/private/exclusive" o "what are you into" o "what do you do in private" → NUNCA listes servicios como menú (NO: "nude, toys, deepthroat, anal..."). Describe SENSACIONES y vibes, hazlo sentir qué va a EXPERIMENTAR. Ej EN: "mmm its just u n me bby... i go crazy wild no holding back 🔥" / Ej ES: "mmm ahí somos solo tu y yo bb... me suelto toda y no paro hasta q no puedas más 🔥". Limits menciónalo casual.
...
{"response":"texto","translation_es":"traducción"}`;
  ...
  try {
   ...
   // === DETECCIÓN DE IDIOMA + CHATBOX LOVENSE ===
    const isChatbox = /Impulsado por Chatbox|Powered by Chatbox/i.test(message);
    const cleanMessage = message.replace(/-\s*(Impulsado por Chatbox|Powered by Chatbox)/gi, '').trim();

    const isEnglishMsg = /^[a-zA-Z0-9\s.,!?'":;\-@#$%&*()\[\]{}\/\\💋🔥❤️😏😍🤤]+$/.test(cleanMessage);
    const isSpanishMsg = /[áéíóúñ¿¡]/.test(cleanMessage) || /\b(hola|como|quiero|amor|papi|rico|donde|eres|bien|dame|hazlo|para|tengo|puedo)\b/i.test(cleanMessage);

    let langHint = '';
    if (isChatbox) {
      langHint = "\n\nIMPORTANT: This message comes from Lovense Chatbox (auto-translated). The fan ORIGINALLY wrote in ENGLISH. You MUST respond 100% in ENGLISH. Zero spanish words.";
